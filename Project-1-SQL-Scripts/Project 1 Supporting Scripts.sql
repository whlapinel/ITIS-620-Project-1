...
-- SUPPORTING QUERY 9: Search for patients based on name
SELECT * 
FROM patients 
WHERE full_name = 'John Doe';

-- SUPPORTING QUERY 10: Search for patients based on ID
SELECT * 
FROM patients 
WHERE patient_id = 2;

-- SUPPORTING QUERY 11: search for records based on visit date:
SELECT * 
FROM patients 
...
-- SUPPORTING QUERY 12: List all patients who have been given a certain diagnosis
SELECT *
FROM patients
JOIN medical_records ON patients.patient_id = medical_records.patient_id
WHERE medical_records.diagnosis = 'Myopia';

-- SUPPORTING QUERY 13: List all patients who visited on certain days
SELECT *
FROM patients
JOIN medical_records ON patients.patient_id = medical_records.patient_id
JOIN visits on medical_records.visit_id = visits.visit_id
...
-- SUPPORTING QUERY 14: List all patients who have been seen by a certain doctor
SELECT *
FROM patients
JOIN medical_records ON patients.patient_id = medical_records.patient_id
JOIN providers ON medical_records.provider_id = providers.provider_id
WHERE providers.full_name = 'Dr. Iris Clearview';

-- SUPPORTING QUERY 15: List all patients who have visited the clinic twice within the shortest time interval
